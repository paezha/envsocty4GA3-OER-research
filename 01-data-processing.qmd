...
reflections_vocab <- prune_vocabulary(reflections_vocab, term_count_min = 5)
```
...
# NOTE: this needs to be updates
text_tbl <- data.frame(
  Terms = c("helped", "confidence", "enjoyed", "easy", "valuable", "excited"),
  Context = c(
    ...
    "Assignments were done on a weekly basis... This was a fantastic idea because other classes that I have taken cover a lot of material in between testing, and it is **easy** for students to lose grasp of those concepts or forget them entirely",
...
# The graphs suggest that there are 14-17 topics in the corpus, but this could be difficult to interpret
# Topics could be explored starting with k = 7
```

```{r, cache = TRUE, include=FALSE}
reflections_lda <- LDA(reflections_dtm, k = 7, method = "Gibbs", control = NULL, model = NULL)
```
